<template>
  <div>
    ...
    >
      <template #header>
        <h3 style="margin: 0px">
          <i class="pi pi-stop" style="font-size: 20px" /> Nueva subcategoría
        </h3>
      </template>
...
export default {
  ...
  methods: {
    ...
    async guardar() {
      ...
      await this.axios.post("/api/subcategoria", params).then((response) => {
        console.log(response.data);
        if (response.data.code == 200) {
          this.$toast.add({
            severity: "success",
            summary: "Mensaje de confirmación",
            detail: "Subcategoría creada con éxito",
            life: 3000,
          });
