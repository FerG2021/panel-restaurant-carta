<template>
  <div>
    ...
    >
      <template #header>
        <h3 style="margin: 0px">
          <i class="pi pi-stop" style="font-size: 20px" /> Modificar
          subcategoría
        </h3>
      </template>
...
export default {
  ...
  methods: {
    ...
    async getDatos() {
      this.loading = true;
      await this.axios.get("/api/subcategoria/" + this.id).then((response) => {
    ...
    async guardar() {
      ...
      await this.axios
        .put("/api/subcategoria/" + this.id, params)
        .then((response) => {
          console.log(response.data);
          if (response.data.code == 200) {
            this.$toast.add({
              ...
              detail: "Subcategoría modificada con éxito",
              life: 3000,
            });
